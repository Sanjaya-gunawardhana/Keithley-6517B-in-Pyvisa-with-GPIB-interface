...
keithley.write(':SENSE:FUNC "CHAR"')  # Set to charge measurement mode
keithley.write(':SENSE:CHAR:RANGE 200E-9')  # Set charge range (e.g., 200 nC range)
keithley.write(':SENSE:CHAR:NPLC 0.01')  # Set NPLC
keithley.write(':SENS:CHAR:MED:STAT OFF')  # Turn off median filtering
keithley.write(':SYST:ZCH OFF')  # Disable zero check


# Disable the display for better performance
...
for entry in filtered_data:
    if entry[0] != ',':
        entry = ',' + entry
    parts = entry.split(',')
    NCOUL_value = parts[1].replace('NCOUL', '').strip()
    secs_value = parts[2].replace('secs', '').strip()
    current.append(float(NCOUL_value))  # Convert to float
